...
class RPCEvent : public Client2ServerEvent,
 ...
 public:
  ...
  /**
   * @fn initialize
   * @brief When the client calls `initialize`, then this function will be
   * called.
   */
  ...
  /**
   * @fn push_message_request
   * @brief When the server wants to push back `push_message` to the client,
   * call this function.
   */
  virtual void push_message_request();

 private:
  // These are the contact point with the clients when pushing back any events
  // from the server.
