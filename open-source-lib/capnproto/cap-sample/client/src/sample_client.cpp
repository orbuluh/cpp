...
SampleClient::SampleClient()
  ...
  m_Address = m_SendAsyncIoContext.provider->getNetwork()
                  .parseAddress("unix:sample.sock")
                  .wait(m_SendAsyncIoContext.waitScope);
...
void SampleClient::start() {
  m_ReceiveThread = std::thread([&]() {
    ...
    kj::Own<capnp::TwoPartyClient> rpc_client =
        kj::heap<capnp::TwoPartyClient>(*connection);
        ...
        std::accumulate(TARGET_SIGNALS.begin(), TARGET_SIGNALS.end(),
                        ...
                        [&](kj::Promise<siginfo_t>& _on_signal, int signal) {
                          return _on_signal.exclusiveJoin(
                              async_io.unixEventPort.onSignal(signal));
                        })
            .then([](siginfo_t a) {
...
void SampleClient::subscribe(kj::AsyncIoContext& async_io,
                             kj::Own<capnp::TwoPartyClient>& rpc_client) {
  std::cout << "[CLIENT]" << __PRETTY_FUNCTION__ << " called" << std::endl;
  ...
  m_SubscriberLifeCycle.push_back(
      subscribe_request.send().wait(async_io.waitScope));
}

bool SampleClient::is_running() { return m_IsRunning; }
